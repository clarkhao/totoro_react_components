...
export declare const useToastStore: import("zustand").UseBoundStore<
  Omit<
    Omit<import("zustand").StoreApi<TState & Action>, "subscribe"> & {
      subscribe: {
        (
          listener: (
            selectedState: TState & Action,
            previousSelectedState: TState & Action,
          ) => void,
        ): () => void;
        <U>(
          selector: (state: TState & Action) => U,
