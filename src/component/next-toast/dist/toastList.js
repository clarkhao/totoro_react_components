...
function ToastList() {
  var _a = useToastStore(function (state) {
      return [state.toastMsg, state.setToastMsg];
    }),
    toastMsg = _a[0],
    setToastMsg = _a[1];
  return React.createElement(
    ...
    {
      className:
        "w-full flex flex-col justify-end items-center absolute left-0 right-0 bottom-0 ml-auto mr-auto",
    },
    React.createElement(
      TransitionGroup,
      { component: null },
      toastMsg.map(function (toast) {
        return React.createElement(
          CSSTransition,
          {
            key: toast === null || toast === void 0 ? void 0 : toast.id,
            classNames: "toast",
            timeout: 500,
          },
          React.createElement(
            ...
            React.createElement(
              "button",
              {
                type: "button",
                className:
                  "ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700",
                "data-dismiss-target": "#toast-success",
                "aria-label": "Close",
                onClick: function () {
                  return setToastMsg(function (prevToasts) {
                    return prevToasts.filter(function (el) {
                      return (
                        el.id !==
                        (toast === null || toast === void 0 ? void 0 : toast.id)
                      );
                    });
                  });
                },
